...
def check(url):
    ...
    if acao:
        ...
        if acao == "*":
            ...
            alert(
                url, "Any origin allowed"
            )  # firefox/chrome/safari/opera only
            ...
            alert(url, "Pre-domain wildcard") if sld(host) else warning(
                url, "Pre-subdomain wildcard"
            )
            ...
            info(url, acao)
    elif acao is not None and not args.q:
...
def cors(url, origin, ssltest=False, firstrun=False):
    ...
    if origin != "null":
        origin = (
            "http://" if (ssltest or not args.s) else "https://"
        ) + origin
    try:
        ...
        acac = (
            str(
                response.info().get("Access-Control-Allow-Credentials")
            ).lower()
            == "true"
        )
        ...
        if firstrun:
            ...
            if vary and not args.q:
                warning(
                    url, "Access-Control-Allow-Origin dynamically generated"
                )
        if ssltest and response.info().get("Strict-Transport-Security"):
            acao = ""
        return (acao or "") if acac else ""
    except Exception as e:
        if not args.q:
            error(url, str(e) or str(e).splitlines()[-1])
        if not firstrun:
            return ""
...
def error(url, msg):
    print(url, "- Error:", msg, file=sys.stderr)
...
def invalid(url, msg):
    print(url, "- Invalid:", msg)


def warning(url, msg):
    print(url, "- Warning:", msg)


def notvuln(url, msg):
    if args.v:
        print(url, "- Not vulnerable:", msg)


def info(url, msg):
    print(url, "- Access-Control-Allow-Origin:", msg)


def usage():
    parser = argparse.ArgumentParser(
        description="Simple CORS misconfigurations checker"
    )
    parser.add_argument("infile", help="File with domain or URL list")
    parser.add_argument(
        "-c", metavar="name=value", help="Send cookie with all requests"
    )
    parser.add_argument(
        "-p", metavar="processes", help="multiprocessing (default: 32)"
    )
    parser.add_argument(
        "-s", help="always force ssl/tls requests", action="store_true"
    )
    parser.add_argument(
        "-q", help="quiet, allow-credentials only", action="store_true"
    )
    parser.add_argument(
        "-v", help="produce a more verbose output", action="store_true"
    )
...
def main():
    ...
    try:
        pool.map_async(check, urls).get(2 ** 32)
